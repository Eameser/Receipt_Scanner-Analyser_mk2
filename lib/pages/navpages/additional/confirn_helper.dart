...
class WordBox{
  ...
  WordBox({this.text, this.vertices, this.boundingBox, this.lineNum});
...
class ConfirmHelper {
  ...
  static getItems(List lines) {
    ...
    //RegExp regExp = RegExp(r'\p{Sc}', unicode: true); //$
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (moneyExp.hasMatch(lines[i]) &&
        ...
        if (calcTotal + lineCost <= scanTotal) {
          ...
          items.add(Item(lines[i], lineCost, lineCost/quantity, quantity));
        }
      }
    }
    ...
    //         finalTotal = scanTotal;
    //     }
    return (ReceiptInfo(' ', 0, items, finalTotal, finalTax));
  }
